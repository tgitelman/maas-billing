...
echo "▶ Wiring metrics in OPS_NS=${OPS_NS} (APP_NS=${APP_NS})"

# 1) Ensure the operators namespace is opted into User Workload Monitoring
echo "→ Labeling namespace '${OPS_NS}' with openshift.io/user-monitoring=true"
oc label namespace "${OPS_NS}" openshift.io/user-monitoring=true --overwrite >/dev/null

# 2) Ensure Authorino *metrics* Service is labeled for selection
AUTH_SVC="authorino-controller-metrics"
echo "→ Verifying Service '${AUTH_SVC}' exists in ${OPS_NS}"
if ! oc -n "${OPS_NS}" get svc "${AUTH_SVC}" >/dev/null 2>&1; then
  die "Service ${OPS_NS}/${AUTH_SVC} not found. Is Authorino installed?"
fi

echo "→ Labeling ${OPS_NS}/${AUTH_SVC} with maas.observability/authorino-controller=true"
oc -n "${OPS_NS}" label svc "${AUTH_SVC}" maas.observability/authorino-controller=true --overwrite >/dev/null

# 3) Create/Update a focused ServiceMonitor that matches ONLY that metrics Service
SM_NAME="authorino-controller-sm"
echo "→ Applying ServiceMonitor ${OPS_NS}/${SM_NAME} (idempotent)"
cat <<'YAML' | sed "s/{{OPS_NS}}/${OPS_NS}/g" | oc apply -f - >/dev/null
...
for S in authorino-authorino-authorization authorino-authorino-oidc; do
  if oc -n "${OPS_NS}" get svc "$S" >/dev/null 2>&1; then
    if oc -n "${OPS_NS}" get svc "$S" -o jsonpath='{.metadata.labels.monitoring\.kuadrant\.io/scrape}' 2>/dev/null | grep -q .; then
      echo "→ Removing monitoring.kuadrant.io/scrape from ${OPS_NS}/${S}"
      oc -n "${OPS_NS}" label svc "$S" monitoring.kuadrant.io/scrape- >/dev/null || true
    fi
  fi
done

# 5) (Optional) Limitador is already exposed correctly by its own ServiceMonitor in OPS_NS.
#    We deliberately don't touch it unless needed.

echo "✅ Metrics wiring complete."
echo "   - Prometheus (UWM) should now scrape ${OPS_NS}/${AUTH_SVC} at /metrics."
