...
- id: multitenant-envoyfilter
  content: "Per-tenant EnvoyFilter: PR #1251 (supersedes #1172). Rebased on upstream/main (2026-08-03) after #1292 bootstrap race fix, #1272 MaasTenantConfig readiness gate, and #1295 payload-processing priority landed. Reimplemented atop new AITenantReconciler structure: EF step placed after tenantConfigReady gate (gateway guaranteed to exist), Config watch coexists with MaasTenantConfig watch (not replaces), cleanup in reconcileAITenantDelete after deleteAITenantScopedChildren. Each tenant gets maas-model-access-logs-<tenant> targeting its gateway via workloadSelector (Istio 1.26+ compat — targetRefs removed). envoyFilterTenantID helper, RBAC get;create;patch;delete, mapConfigToAITenants namespace-scoped, cross-namespace OwnerRef documented, transient delete errors returned. ObservabilityReady status condition on AITenant surfaces state (MonitoringNamespaceNotConfigured / UsageLoggingDisabled / EnvoyFilterApplied). 280+ lines of tests. 6 files, +418/-211. Updated 2026-08-04: workloadSelector migration (PR #1313 compat), ObservabilityReady condition (ahadas review feedback)."
  status: done
...
- id: lokiLogQLVariable-user-dropdown
  ...
  status: done
- id: pr-step-fix
  content: "Branch feature/step-fix pushed (2026-07-22). PR #1242 created. Increases tokenConsumptionOverTime range vector from [30m] to [2h] on both dashboards. Workaround for Loki split_queries_by_interval: 30m. Panel description cleaned up (implementation details moved to YAML comments)."
  status: done
...
**Per-tenant EnvoyFilter** (PR #1251, supersedes #1172): Clean reimplementation from `upstream/main` with all review feedback. Moved EnvoyFilter lifecycle from `LifecycleReconciler` into `AITenantReconciler`. Each AITenant gets its own EnvoyFilter (`maas-model-access-logs-<tenant>`) targeting its specific gateway via `workloadSelector` (Istio 1.26+ compatible — `targetRefs` removed). `AITenantReconciler` reads `Config` CR directly for `usageLogging` flag and watches Config changes to propagate toggles via `mapConfigToAITenants` (namespace-scoped). Changes vs #1172: `envoyFilterTenantID` helper centralizes default→"" mapping, RBAC includes `get` verb, `deleteEnvoyFilterIfExists` takes `efName string` (not `*unstructured`), `applyUsageLogsEnvoyFilter` separated from `ensureUsageLogsEnvoyFilter`, cross-namespace OwnerRef documented, transient cleanup errors returned for retry, collector address uses `.svc` (not `.svc.cluster.local`). Updated 2026-08-04: `patchEnvoyFilterWorkloadSelector` replaces `patchEnvoyFilterTargetGateway` (aligns with PR #1313 / Istio 1.26 `workloadSelector`-only mandate). `ObservabilityReady` status condition on AITenant surfaces observability state (`MonitoringNamespaceNotConfigured` / `UsageLoggingDisabled` / `EnvoyFilterApplied`) per ahadas review. 280+ lines of tests covering disabled/enabled/named-tenant/delete/workloadSelector/naming/condition.
...
- **Enable** (`usageLogging: true`): reads EnvoyFilter YAML from container filesystem (`/deployment/components/observability/usage-logs/envoy-otel-access-log.yaml`), templates collector address via `patchClusterAddress`, patches `spec.workloadSelector.labels["gateway.networking.k8s.io/gateway-name"]` to the tenant's gateway (`patchEnvoyFilterWorkloadSelector`), removes any leftover `spec.targetRefs` (Istio 1.26+ mutual exclusion), names the resource via `UsageLogsEnvoyFilterName(envoyFilterTenantID(aitenant))`, sets Config ownerReference (cross-namespace, documented), server-side applies with `maas-controller` fieldOwner. Sets `ObservabilityReady=True` condition with reason `EnvoyFilterApplied`.
- **Disable** (`usageLogging: false` or field absent): deletes the per-tenant EnvoyFilter if it exists. Sets `ObservabilityReady=False` condition with reason `UsageLoggingDisabled`.
...
- **Graceful degradation**: skips if EnvoyFilter CRD not installed, manifest file not found, or `gatewayRef` not yet populated. If `MonitoringNamespace` is empty, sets `ObservabilityReady=False` condition with reason `MonitoringNamespaceNotConfigured` and returns early (no EnvoyFilter created).
...
Files changed (per-tenant EnvoyFilter, PR #1251, rebased 2026-08-03, updated 2026-08-04):
- `maas-controller/api/maas/v1alpha1/aitenant_types.go` — added `AITenantConditionObservabilityReady = "ObservabilityReady"` constant
- `maas-controller/pkg/controller/maas/aitenant_controller.go` — `envoyFilterTenantID`, `ensureUsageLogsEnvoyFilter` (sets `ObservabilityReady` condition: `MonitoringNamespaceNotConfigured`/`UsageLoggingDisabled`/`EnvoyFilterApplied`), `deleteEnvoyFilterIfExists`, `applyUsageLogsEnvoyFilter`, `patchEnvoyFilterWorkloadSelector` (replaces `patchEnvoyFilterTargetGateway` — uses `workloadSelector.labels` + removes leftover `targetRefs` for Istio 1.26+ compat); Config watch in `SetupWithManager` + `mapConfigToAITenants` (namespace-scoped, coexists with MaasTenantConfig watch from #1272); EnvoyFilter step placed after `tenantConfigReady` gate (gateway guaranteed to exist); EnvoyFilter cleanup in `reconcileAITenantDelete` after `deleteAITenantScopedChildren` (transient errors returned); RBAC marker `get;create;patch;delete`
- `maas-controller/pkg/controller/maas/aitenant_controller_test.go` — 280+ lines: `efManifestPath` helper, `TestAITenantEnsureUsageLogsEnvoyFilter` (disabled+condition, monitoring-ns-empty+condition, default tenant+condition, named tenant, delete), `TestPatchEnvoyFilterWorkloadSelector` (legacy targetRefs→workloadSelector, existing workloadSelector patch), `TestUsageLogsEnvoyFilterName`
...
- **`dashboard-4-maas-usage-logs-admin`** — admin view, all users, uses `usage-logs-all` datasource (direct to LokiStack). User filter via `LokiLogQLVariable` (auto-populated dropdown from structured metadata via `count by (user_id) | keep user_id`, independent of subscription/model, follows dashboard time picker via `[$__range]`). Subscription dropdown filters `subscription!="-"`. Model dropdown filters `model!="-", subscription!="-"` (proxy for unresolved identity — WASM shim is atomic, `subscription="-"` ↔ `user_id="-"`). Table queries filter `| user_id!="-"` to exclude auth-failure rows.
- **`dashboard-5-maas-usage-logs`** — per-user view, uses `usage-logs-multi-tenancy` datasource (through `usage-logs-tenancy-proxy`). Auto-filtered by user identity. Subscription/Model dropdowns filter `subscription!="-"`. Table queries filter `subscription!="-"` in stream selectors (no `user_id` column — proxy handles user scoping).
...
**Table panel**: Both dashboards have a "Usage breakdown" table with four Loki queries merged via `MergeSeries` transform. All table queries use `[$__range]` (negative offset removed — not supported by this Loki version). Admin table groups by `user_id, model, subscription` and filters `| user_id!="-"` (excludes auth-failure rows). User table groups by `model, subscription, key_name` and filters `subscription!="-"` in stream selectors. All four queries are cross-zero-padded so every label combo appears in every query (0 instead of empty — required for correct MergeSeries join and sorting):
- **Q1**: Tokens — `sum_over_time(... response_type="hit" ... | unwrap tokens_total [$__range]) or (error * 0)`
- **Q2**: Successful requests — `count_over_time(... response_type="hit" ... [$__range]) or (error * 0)`
- **Q3**: Rate-limited requests — `count_over_time(... response_type="rate_limit" ... [$__range]) or (hit * 0) or (error * 0)`
- **Q4**: Failed requests — `count_over_time(... response_type="error" ... [$__range]) or (hit * 0)`

The `or (other_query * 0)` pattern generates a 0-valued series with matching labels for every label combo that exists in the other query but not in the current one. This ensures all four queries return identical label sets — `MergeSeries` places `0` in cells instead of empty, and sorting works correctly (numbers before empty).
...
| Dashboard: `subscription!="-"` on dropdowns | Model dropdown matchers filter `subscription!="-"` (proxy for unresolved identity — WASM shim is atomic: auth failure → both `user_id="-"` and `subscription="-"`). Subscription dropdown already filters `subscription!="-"`. This excludes auth-failure entries (401/403) where model names are garbage (client-provided, not routed). Error entries with resolved identity (e.g. 404 model-not-found after successful auth) remain visible. |
...
### `aitenant_controller.go` EnvoyFilter Constants (2026-07-23, rebase verified 2026-08-03, updated 2026-08-04)

PR #1251: `envoyFilterManifestPath` const now lives in `aitenant_controller.go` (moved from `self_deployment_controller.go` which no longer manages EnvoyFilters). `patchClusterAddress` remains in `self_deployment_controller.go` (shared across both controllers — same Go package). After rebase on upstream/main (2026-08-03), the placement adapts to the new `Reconcile()` structure from #1292 (bootstrap race fix): `ensureUsageLogsEnvoyFilter` runs after the `tenantConfigReady` gate, ensuring the gateway exists before creating the EF. Updated 2026-08-04: `patchEnvoyFilterTargetGateway` replaced by `patchEnvoyFilterWorkloadSelector` to align with PR #1313 (Istio 1.26 made `targetRefs` and `workloadSelector` mutually exclusive). `AITenantConditionObservabilityReady` constant added to `aitenant_types.go`.
...
**Note on "Total requests" vs table total**: The "Total requests" stat panel query uses no `response_type` filter — it counts all log lines (200 + 429 + 403 + other) including auth failures. The table filters `user_id!="-"` (admin) / `subscription!="-"` (user) to exclude auth-failure rows, and shows four columns: Tokens used, Successful requests, Rate limited requests, and Failed requests. Auth-failure entries (401/403 with unresolved identity) only appear in the stat panel totals, not in the table. This is correct — the success rate formula `hit / all` accounts for all response types.
...
- **`user_id!="" | user_id!="-"`**: Excludes sentinel values from dropdown. Dash (`-`) appears on auth-failure entries (401/403) where the WASM shim didn't store identity in FILTER_STATE (auth is atomic — all-or-nothing).
...
Four patches applied to the tenant's gateway via `workloadSelector` (Istio 1.26+):
...
| [#1251](https://github.com/opendatahub-io/models-as-a-service/pull/1251) Per-tenant EnvoyFilter | `feature/aitenant-envoy-filter-v2` | Open (rebased, updated 2026-08-04) | Move EnvoyFilter from LifecycleReconciler to AITenantReconciler. Supersedes #1172. Rebased implementation (2026-08-03) adapts to #1292 bootstrap race fix + #1272 MaasTenantConfig readiness gate: EF step after tenantConfigReady, Config watch coexists with MaasTenantConfig watch. Updated (2026-08-04): workloadSelector migration (PR #1313 / Istio 1.26 compat), ObservabilityReady status condition (ahadas review). 280+ lines of tests. | 7 files, +450/-211 | #1035 (merged), #1203 (merged), #1313 (merged). Conflicts with #1292, #1272, #1295 resolved. |
...
6. **Multi-tenant EnvoyFilter** (jrhyness, PR #1035 review): **PR #1251 open** (supersedes #1172). Rebased 2026-08-03 on upstream/main after #1292, #1272, #1295 landed. Updated 2026-08-04: `patchEnvoyFilterWorkloadSelector` replaces `patchEnvoyFilterTargetGateway` (aligns with PR #1313 / Istio 1.26 `workloadSelector`-only mandate), `ObservabilityReady` status condition on AITenant (ahadas review: surfaces state visibly on the resource instead of silent log). EF step runs after `tenantConfigReady` gate, Config watch coexists with MaasTenantConfig watch. Key design unchanged: `AITenantReconciler` manages per-tenant EnvoyFilter with `envoyFilterTenantID` helper, RBAC `get;create;patch;delete`, `mapConfigToAITenants` namespace-scoped, transient cleanup errors returned for retry, cross-namespace OwnerRef documented. 280+ lines of tests. **Next**: force-push updated branch, re-trigger CI, respond to ahadas on telemetry settings design scope.
