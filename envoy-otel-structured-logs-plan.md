...
6. **Usage Dashboard — Perses Loki plugin limitations (investigated + validated 2026-03-31; doc refreshed 2026-05-14)**:
  The Usage dashboard relies on Perses built-in **`$__range`** for LogQL lookback on stat panels and the table (see `deployment/components/observability/perses/dashboards/dashboard-usage.yaml` and [Perses Variable concepts](https://perses.dev/perses/docs/concepts/variable/)). **Five upstream gaps** in the Loki plugin / `query_range` / console rendering interaction are material: **A–C** were validated on-cluster in 2026-03-31; **D** documents spec vs OpenShift Console (2026-05-14); **E** documents `columnSettings` ghost columns preventing variable-driven table grouping (2026-05-17). Official `LokiTimeSeriesQuery` surface: [Loki plugin model](https://perses.dev/plugins/docs/loki/model/).
  ...
  **Mitigation in this repo:** `docs/samples/dashboards/grafana-usage-loki-token-mock.json` imports into **Grafana** with `fillOpacity`, line width, and stacking for the same LogQL as the Usage **Token consumption** panel when filled ribbons are required for design review.
  ### Limitation E: Table `columnSettings` renders headers for labels absent from query data (2026-05-17)
  **Root cause:** The Perses Table plugin creates a column for every entry in `columnSettings`, regardless of whether the query result contains that label. `hide: <boolean>` is a static field — it cannot reference a variable. There is no conditional column visibility.
  **Impact on View by:** When the table query uses `sum by (${view_by:raw})`, selecting **By model** returns only the `model` label — the `subscription` column (defined in `columnSettings`) renders with a header but no values (empty column), and vice versa. This makes the table look broken even though the aggregation is correct.
  **Approaches tried and rejected:**

  | Approach | Result |
  | --- | --- |
  | `sum by (${view_by:raw})` on table | Correct aggregation but empty Model or Subscription column |
  | Hidden paired variable (`view_by_chart_drop`) for chart, separate table query | OpenShift Console Perses does not sync hidden `StaticListVariable` when the visible variable changes |
  | `MergeColumns` transform (merge model + subscription into "Name") | Works but loses distinct Model/Subscription column headers |
  | Remove model/subscription from `columnSettings` (auto-discover) | Dimension column appears last (after Tokens/Requests); position not controllable |
  | `sum by (model, subscription)` vs `sum by (subscription, model)` for View by | Same label set, same rows — selector appears to do nothing |
