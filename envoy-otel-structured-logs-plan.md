...
6. **CR API version**: CRs use `perses.dev/v1alpha1` — COO 1.5 warns deprecated, update to `v1alpha2` next cycle. Requires restructuring YAML (`spec.display`/`panels`/`variables` → `spec.config.*`). Conversion webhook handles it automatically for now.
